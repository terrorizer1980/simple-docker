...
class Currency {
    constructor(type, amount, daiRate) {
        ...
        this.getRate = (currency) => {
            const exchangeRates = {
                ...
                DEI: this.toRay(utils_1.parseUnits(this.daiRate, 18).toString()),
                ETH: this.toRay("1"),
                FIN: this.toRay(utils_1.parseUnits("1", 3).toString()),
                WEI: this.toRay(utils_1.parseUnits("1", 18).toString()),
            };
        ...
        this.toWad = (n) => utils_1.parseUnits(n.toString(), 18);
        this.toRay = (n) => this.toWad(this.toWad(n.toString()));
        this.fromWad = (n) => utils_1.formatUnits(n.toString(), 18);
        ...
        try {
            this.wad = this.toWad(amount._hex ? utils_1.bigNumberify(amount._hex) : amount);
            this.ray = this.toRay(amount._hex ? utils_1.bigNumberify(amount._hex) : amount);
        }
        catch (e) {
    ...
    toBN() {
        return utils_1.bigNumberify(this._round(this.amount));
    }
    format(_options) {
        ...
        const amount = options.round
            ...
            : options.decimals > nDecimals
                ...
                : options.decimals < nDecimals
                    ...
                    : amt;
        return `${symbol}${options.commas ? utils_1.commify(amount) : amount}`;
    }
    round(decimals) {
        const amt = this.amount;
        const nDecimals = amt.length - amt.indexOf(".") - 1;
        ...
        if (typeof decimals === "number" && decimals < nDecimals) {
            const roundUp = utils_1.bigNumberify(`5${"0".repeat(18 - decimals - 1)}`);
