...
const deployContract = async (name, artifacts, args) => {
    ...
    if (!addressBook[chainId])
        addressBook[chainId] = {};
    if (!addressBook[chainId][name])
        addressBook[chainId][name] = {};
    addressBook[chainId][name] = Object.assign({ address, creationCodeHash, runtimeCodeHash, txHash }, saveArgs);
    saveAddressBook(addressBook);
    return contract;
};
const sendGift = async (address, token) => {
    const ethGift = "100000";
...
const migrate = async () => {
    ...
    provider = new eth.providers.JsonRpcProvider(process.env.ETH_PROVIDER, classicProviders.includes(process.env.ETH_PROVIDER) ? "classic" : null);
    mnemonic = process.env.ETH_MNEMONIC;
    wallet = eth.Wallet.fromMnemonic(mnemonic).connect(provider);
    try {
...
//# sourceMappingURL=migrate-contracts.js.map